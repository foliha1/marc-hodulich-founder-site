...
// ===== TEMP DIAGNOSTIC (remove after identifying the signing construction) =====
async function hmacHex(alg: string, key: string, message: string): Promise<string> {
  const cryptoKey = await crypto.subtle.importKey(
    'raw',
    encoder.encode(key),
    { name: 'HMAC', hash: alg },
...
async function logSignatureDiagnostic(
  key: string,
  rawBody: string,
  provided: string,
  ...
  const charset = /^[0-9a-f]+$/i.test(normalized)
    ? 'hex'
    : /^[A-Za-z0-9+/_-]+={0,2}$/.test(normalized)
      ? 'base64'
      : 'other'
  const decoded = decodeSignature(provided)
  const providedHex = decoded
    ? [...decoded].map((b) => b.toString(16).padStart(2, '0')).join('')
    : null
  ...
  const messages: Record<string, string> = {
    ...
    'php(raw-parsed)': (() => {
      try {
        return phpJsonEncode(JSON.parse(rawBody))
      } catch {
        return rawBody
      }
    })(),
  ...
  for (const alg of ['SHA-1', 'SHA-256', 'SHA-512']) {
    for (const [name, message] of Object.entries(messages)) {
      const hex = await hmacHex(alg, key, message)
      if (providedHex && hex === providedHex) matches.push(`${alg}/${name}`)
    }
  }

  console.log('storychief-webhook DIAG', {
    signature_length: normalized.length,
    signature_charset: charset,
    decoded_byte_length: decoded?.length ?? null,
    timestamp_value: timestamp,
    body_bytes: rawBody.length,
    body_preview: rawBody.length <= 512 ? rawBody : '[omitted]',
    matches: matches.length ? matches : 'none',
  })
}
// ===== END TEMP DIAGNOSTIC =====
